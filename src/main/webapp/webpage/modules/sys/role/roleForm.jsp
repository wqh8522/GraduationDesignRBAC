...
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="role" autocomplete="on" action="${ctx}/sys/role/save" method="post" class="form-horizontal" >
		<form:hidden path="id"/>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      ...
		      <tr>
		         ...
		         <td  class="width-15 active" class="active">
		         ...
		         <td class="width-35">
	         		<form:select path="dataScope" class="form-control ">
	         			<c:if test="${userscope*1 <= 1}">
	         				<option value="1" ${role.dataScope == 1?'selected':''}>${ fns:getDictLabel ("1", "sys_data_scope", "1")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 2}">
	         				<option value="2" ${role.dataScope == 2?'selected':''}>${ fns:getDictLabel ("2", "sys_data_scope", "2")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 3}">
	         				<option value="3" ${role.dataScope == 3?'selected':''}>${ fns:getDictLabel ("3", "sys_data_scope", "3")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 4}">
	         				<option value="4" ${role.dataScope == 4?'selected':''}>${ fns:getDictLabel ("4", "sys_data_scope", "4")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 5}">
	         				<option value="5" ${role.dataScope == 5?'selected':''}>${ fns:getDictLabel ("5", "sys_data_scope", "5")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 8}">
	         				<option value="8" ${role.dataScope == 8?'selected':''}>${ fns:getDictLabel ("8", "sys_data_scope", "8")}</option>
	         			</c:if>
	         			<c:if test="${userscope*1 <= 9}">
	         				<option value="9" ${role.dataScope == 9?'selected':''}>${ fns:getDictLabel ("9", "sys_data_scope", "9")}</option>
	         			</c:if>
					</form:select>
